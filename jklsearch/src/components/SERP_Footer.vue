<template>
    <footer id="footer" class="flex items-center justify-between footer items-center justify-between footer-center p-6 bg-base-200 text-base-content rounded">
      <div class="grid grid-flow-col gap-4">
        <a class="link link-hover">About us</a> 
        <a class="link link-hover">Product</a> 
        <a class="link link-hover">Support</a> 
        <a class="link link-hover">Blog</a>
      </div> 
      <div class="p-15 grid grid-flow-col gap-3">
        <a class="link link-hover">Privacy</a> 
        <a class="link link-hover">Terms</a> 
        <a class="link link-hover">Settings</a> 
      </div> 
    </footer>
    
</template>
<style>
  #footer {
    position: fixed;
    bottom: 0;
    width: 100%;
    height: 1%;   /* Height of the footer */
    /*background: #6cf;*/
  }
</style>